...
export const ConnectDevice = ({ child, onChange }: Props) => {
  ...
  const rotateToken = async () => {
    if (!confirm(t("connect.rotateConfirm"))) return;
    ...
    toast.success(t("connect.tokenRotated"));
    onChange();
  };

  const onFile = async (file: File) => {
    if (file.size > 2_000_000) return toast.error(t("connect.max2mb"));
    setUploading(true);
    try {
      ...
      toast.success(t("connect.imported", { n: (data as any).ingested, d: (data as any).days.length }));
      onChange();
    } catch (e: any) {
      toast.error(e.message ?? t("connect.importError"));
    } finally {
      setUploading(false);
  ...
  return (
    <Card className="p-6">
      <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
        <div>
          <h3 className="font-semibold flex items-center gap-2"><Zap className="h-4 w-4 text-primary" /> {t("connect.title")}</h3>
          <p className="text-sm text-muted-foreground mt-1">
            {t("connect.subtitle", { name: child.name })}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {child.last_ingest_at ? (
            <Badge variant="secondary" className="gap-1">
              ...
              {t("connect.active", { time: new Date(child.last_ingest_at).toLocaleString(i18n.language) })}
            </Badge>
          ) : (
            <Badge variant="outline">{t("connect.noAuto")}</Badge>
          )}
        </div>
      </div>
      ...
      <Tabs defaultValue="api" className="w-full">
        <TabsList className="grid grid-cols-3 w-full">
          <TabsTrigger value="api"><Smartphone className="h-4 w-4 mr-1" /> {t("connect.tabApi")}</TabsTrigger>
          <TabsTrigger value="ios">{t("connect.tabIos")}</TabsTrigger>
          <TabsTrigger value="csv"><Upload className="h-4 w-4 mr-1" /> {t("connect.tabCsv")}</TabsTrigger>
        </TabsList>

        <TabsContent value="api" className="space-y-3 pt-4">
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">{t("connect.endpoint")}</label>
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">{t("connect.tokenLabel", { name: child.name })}</label>
            <div className="flex gap-2">
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">{t("connect.curlExample")}</label>
          ...
          <p className="text-xs text-muted-foreground">
            {t("connect.compatNote")}
          </p>
        </TabsContent>

        <TabsContent value="ios" className="space-y-3 pt-4">
          <ol className="text-sm space-y-2 list-decimal list-inside text-muted-foreground">
            <li>{t("connect.iosStep1")}</li>
            <li>{t("connect.iosStep2")}</li>
            <li>{t("connect.iosStep3")}
              <ul className="ml-6 mt-1 list-disc">
                <li>URL: <code className="text-xs bg-muted px-1 rounded">{INGEST_URL}</code></li>
                <li>{t("connect.iosStep3Method")}</li>
                <li>{t("connect.iosStep3Headers")}</li>
                <li>{t("connect.iosStep3Body")}</li>
              </ul>
            </li>
          </ol>
          <div className="relative">
        ...
        <TabsContent value="csv" className="space-y-3 pt-4">
          ...
          >
            ...
            <p className="font-medium">{t("connect.csvDrop")}</p>
            <p className="text-xs text-muted-foreground mt-1">{t("connect.csvHint")}</p>
            <input ref={fileRef} type="file" accept=".csv,text/csv" className="hidden"
              onChange={e => { const f = e.target.files?.[0]; if (f) onFile(f); }} />
          </div>
          {uploading && <p className="text-sm text-center text-muted-foreground">{t("connect.processing")}</p>}
          <details className="text-xs text-muted-foreground">
            <summary className="cursor-pointer font-medium">{t("connect.expectedFormat")}</summary>
...
2026-04-30,WhatsApp,12`}</pre>
            <p className="mt-2">{t("connect.altHeaders")}</p>
          </details>
        </TabsContent>
      </Tabs>
    </Card>
